...
{
  task->data.ppolyapprox_ptr = ppolyapprox_ptr;

  // Set polynomial degree in CONF0 (bits 29:28)
  task->data.cfg.conf0 &= ~NEUREKA_MASK_FLAG_POLYAPPROX_DEGREE;
  task->data.cfg.conf0 |= ((degree & 0x3) << 28);

  // Set piecewise parameters in CONF1
  task->data.cfg.conf1 &= ~(NEUREKA_MASK_PPOLYAPPROX_NR_PARTS |
                            NEUREKA_MASK_PPOLYAPPROX_MUL_BW |
                            NEUREKA_MASK_PPOLYAPPROX_ADD_BW);
  task->data.cfg.conf1 |= ((nr_active_parts & 0x1f) << 5) |
                          ((mul_bitwidth & 0x3) << 10) |
                          ((add_bitwidth & 0x3) << 12);
}
